...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-4xl font-bold">Add New Movie</h1>
        <a href="{{ route('admin.movies.index') }}" class="px-6 py-3 bg-gray-700 hover:bg-gray-600 rounded font-bold transition">
            ← Back
        </a>
    </div>

    <form action="{{ route('admin.movies.store') }}" method="POST" class="bg-gray-900 rounded-lg p-8">
        @csrf
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="url" name="backdrop" value="{{ old('backdrop') }}" required
                       class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded focus:border-primary focus:outline-none">
                @error('backdrop')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="genre" value="{{ old('genre') }}" placeholder="Action, Sci-Fi, Adventure" required
                       class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded focus:border-primary focus:outline-none">
                @error('genre')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="duration" value="{{ old('duration') }}" placeholder="2h 30m" required
                       class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded focus:border-primary focus:outline-none">
                @error('duration')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="category" required
                    ...
                    <option value="newReleases" {{ old('category') == 'newReleases' ? 'selected' : '' }}>New Releases</option>
                    <option value="action" {{ old('category') == 'action' ? 'selected' : '' }}>Action</option>
                    <option value="scifi" {{ old('category') == 'scifi' ? 'selected' : '' }}>Sci-Fi</option>
                </select>
                @error('category')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="4" required
                          class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded focus:border-primary focus:outline-none">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
